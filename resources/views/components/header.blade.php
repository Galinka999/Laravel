            ...
            @if(\Auth::user())
                <strong class="navbar-brand d-flex align-items-center">
                    <p>Привет, {{ \Auth::user()->name }}!
                        <a href="{{ route('account') }}" class="navbar-brand align-items-center">(Личный кабинет)</a>
                    </p>
                </strong>
            @else
                <strong><a href="{{ route('account') }}" class="navbar-brand d-flex align-items-center">Войти в личный кабинет</a></strong>
            @endif
